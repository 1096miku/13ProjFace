...
| 离线命令词识别率 | ≥90%（安静环境） | **阈值=30 时 ≈46%（37/80）；回退到 20 后 ≈72.5%（58/80）——两者都不达标**；静默 100 s 内**误触发 0 次** | 证据 `build/d7_voice.log`（阈值 30，19:11:06–19:21:11）与 **`build/d7_voice20.log`（阈值 20，19:39:50–19:51，两个窗口都是零复位零重连）**。阈值 20 逐条：温度 6 / 湿度 8 / 光照 6 / 有人 6 / 状态 7 / 提示音 7 / 抓拍 5 / **锁车 13**（超分母，见 BUG-047）。命中概率 **0.20~0.64、多数 0.21~0.40**，说明这批命令词置信度天然偏低，**20 与 30 切的是同一团分布**——阈值这条路已走到头。**唤醒词在阈值 20 下未单独测**（阈值 30 时 10 分钟仅 2 次，用户反馈"喊它也没反应"）。详见 **BUG-046** |
...
2. **命令词阈值**：`CONFIG_CUSTOM_WAKE_WORD_THRESHOLD=30` 后按 8×10 协议重测识别率与误触发 —— **✅ 阈值已改（`sdkconfig` + `sdkconfig.defaults.esp32s3`，2026-09-20 晚）并烧录；测量因板子 USB 掉线中止，待重测**
   - ! 注意：这个阈值是 MN 的**全局**判定门槛，**唤醒词「你好小智」也一起吃它**（`custom_wake_word.cc:120` 的 `set_det_threshold`）。重测时先确认唤醒仍然灵敏
3. **BUG-006**：如果 D7 要做 2 h 无人值守，触摸 NACK 必须不再 abort —— **✅ 已按最小改动降级为"打告警 + 跳过这一帧"（2026-09-20 晚，用户批准；改的是 `managed_components/espressif__esp_lvgl_port/src/lvgl9/esp_lvgl_port_touch.c:126`）**；**"长时间不再 abort"尚未真机验证**，且该文件被 `.gitignore` 忽略（删组件目录即丢），见 `docs/BUGS.md` BUG-006 末尾
